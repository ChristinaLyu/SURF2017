...
if __name__ == "__main__":
    ...
    output_file = os.path.join(output_folder,file_name+"_" + model_index + ".pdb")
    ...
    try:
        ...
        print "ERROR:extract_one_model_pdb_jmol: output folder does not exist and it could not be created."
        sys.exit(-1)

    # run jmol to get the _pr.pdb file
    
    try:
        os.system('java -XX:-UseGCOverheadLimit -jar '+JMOLDATA_JAR+' -n -j '+"'"+'load '+input_file+'; model ' + model_index + '; x=write("PDB"); write VAR x "'+output_file+'";'+"'")
    except IOError:
