...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print 'ERROR:extract_one_chain_pdb_jmol: usage: python extractProtein.py input_pdb_file output_folder'
        sys.exit(-1)

    input_file = sys.argv[1]
    output_folder = sys.argv[2]
    chain_index = sys.argv[3]

    file_name = ntpath.basename(os.path.splitext(input_file)[0])
    # output_dir = os.path.dirname(output_file)
    output_file = os.path.join(output_folder,file_name+"_" + chain_index + ".pdb")
    ...
    try:
        os.system('java -XX:-UseGCOverheadLimit -jar '+JMOLDATA_JAR+' -n -j '+"'"+'load '+input_file+'; select :' + chain_index + '; x=write("PDB"); write VAR x "'+output_file+'";'+"'")
    except IOError:
